...
graph TB
    ...
    subgraph Gateway["API Gateway Layer"]
        APISIX["APISIX<br/>Gateway"]
        Dashboard["Dashboard<br/>UI"]
        RouteManager["Route Manager"]
    end

    subgraph Auth["Authentication Layer"]
        subgraph KC_PQC["Keycloak PQC (Q-Sign)"]
            PQCProv["PQC Provider"]
            VaultInteg["Vault Integration"]
        end
        subgraph KC_HSM["Keycloak HSM (PQC-SSO)"]
            LunaHSM["Luna HSM"]
            HybridJWT["Hybrid JWT"]
        end
    end

    subgraph KeyMgmt["Key Management Layer"]
        subgraph Vault["Q-KMS Vault"]
            Transit["Transit<br/>Engine"]
            KV["KV v2<br/>Secrets"]
            PKI["PKI<br/>Engine"]
        end
        subgraph HSM["Luna HSM Device"]
            FIPS["FIPS 140-2 Level 3<br/>Certified"]
            KeyGen["DILITHIUM3<br/>Key Generation"]
            SignOps["Secure Signing<br/>Operations"]
        end
    end
